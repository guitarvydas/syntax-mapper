...
(defun leaf_new (name handler data ) {
    eh := new(Eh)
    eh.name = name
    eh.handler = handler
    eh.data = data
...
(defun make_message (port_as_string data ) {
    return {
...
(defun make_message_from_string (port s ) {
...
(defun port_clone (port ) {
    return strings.clone (port)
})

// Frees a message.
(defun discard_message_innards (msg ){
...
(defun container_handler (eh message instance_data ){
...
(defun make_fifo_iterator (q ) {
    return {q, 0}
})

(defun fifo_iterate (iter ) {
    if iter.q.len == 0 {
        ok = false
        return
    }
